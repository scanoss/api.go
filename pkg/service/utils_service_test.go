...
func TestDetectCharset(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			// Call the function under test
			charset, err := detectCharset(test.input)
...
func TestDetectCharsetBufferTruncation(t *testing.T) {
	...
	// Call detectCharset
	charset, err := detectCharset(largeBuffer)

	// Verify no error occurred
	assert.NoError(t, err, "Should handle large buffers without error")
	assert.NotEmpty(t, charset, "Charset should not be empty")

	// The detected charset should be one of the ASCII-compatible ones
	validCharsets := []string{"UTF-8", "ISO-8859-1", "ISO-8859-2", "windows-1252", "ISO-8859-15"}
...
func TestDetectCharsetErrorHandling(t *testing.T) {
	...
	charset, err := detectCharset(nil)

	// Should not error, but return UTF-8 due to low confidence
	assert.NoError(t, err, "Should handle nil input gracefully")
